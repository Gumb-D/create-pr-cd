...
- [x] Compare ECC output content, line count, quantities, and key formatting.
  ...
  Defect found (pre-existing, legacy generator) — FIXED with approval:
  `scripts/generate_tss_pr_ecc.py` crashed while writing `REVIEW_REQUIRED_TI_*.csv` when site data contains U+200B (zero-width space) under the default Windows cp1252 file encoding — this affected current production TI runs on the live view data. JJ approved the fix on 2026-07-08: the two review/duplicates CSV writers now use explicit `encoding="utf-8-sig"` (BOM keeps the files Excel-readable). This is the first approved change to the otherwise-frozen generator; ECC output files are unaffected — the parity harness was re-run under the default platform encoding (workaround removed) and golden parity remains **PASS** (TSS 87/87, TI 19/19 identical, TI review CSVs written with BOM). The end-to-end TI test `tests/test_ti_sow_matching.py` exercises the fixed writer and now reads with `utf-8-sig`; full suite `Ran 210 tests` `OK` on 2026-07-08. The Phase 0 "no diff in `generate_tss_pr_ecc.py`" evidence remains true for its 2026-07-06 date and is superseded by this explicitly approved, review-file-only change.
...
- [x] Create separate discovery-only DRAFT profiles for materially different source structures where the strongest evidence already exists.
  Evidence note:
  The repository now contains 10 tracked profiles, initially created DRAFT-only: `tx_mini_pr_v1`, `mw_eos_swap_pr_v1`, `tx_rollout_2023_pr_v1`, `zte_tx_mini_pr_v1`, `jendela_tx_migration_pr_v1`, `celcomdigi_bau_2023_pr_v1`, `celcomdigi_bau_2024_pr_v1`, `celcomdigi_usp_pr_v1`, `cd_consolidation_2023_decom_pr_v1`, and `cd_consolidation_2023_rollout_pr_v1`. `docs/MW_DU_Discovery_Inventory.md` and `config/registries/mw_du_model_discovery_registry.yaml` now surface all 10 profile IDs from the live profile files. Nine of the ten remain in quarantined discovery-only `DRAFT` status; `tx_mini_pr_v1` has since been promoted to `PR_INPUT_READY` (non-production) following completed Phase 1 validation.
