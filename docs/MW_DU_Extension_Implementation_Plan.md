...
- [ ] Review every required canonical PR field against the source export.
  Blocker:
  The field-by-field mapping review is a human decision and is now waiting on JJ to complete the `Decision_Log` sheet in the local TX Mini mapping-decision workbook. Open decisions: select one fingerprint for `tx_sow_raw`, `site_name`, and `antenna_size_ne`; verify the single candidates for `site_code`, `du_key`, `tx_upgrade_scope_raw`, and `antenna_size_fe`; approve a controlled normalization value map for the derived `tx_sow_normalized`; define the PR-reference-to-status normalization transform for `existing_tss_pr_status` / `existing_ti_pr_status`; and decide the canonical-schema treatment for the newly ruled `subcontractor_tss` source field.
