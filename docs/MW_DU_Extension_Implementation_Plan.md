...
- [ ] Review every required canonical PR field against the source export.
  Blocker:
  The field-by-field mapping review is a human decision and is now waiting on JJ to complete the `Decision_Log` sheet in the local TX Mini mapping-decision workbook. Open decisions: select one fingerprint for `tx_sow_raw`, `site_name`, `subcontractor_ti`, and `antenna_size_ne`; verify the single candidates for `site_code`, `du_key`, `tx_upgrade_scope_raw`, `region`, `state`, `latitude`, `longitude`, and `antenna_size_fe`; decide the missing-field treatment for `existing_tss_pr_status` and `existing_ti_pr_status` (all current TX Mini keyword candidates are weak false positives; `2023 TX Rollout` is the profiled donor reference); and approve a controlled normalization value map for the derived `tx_sow_normalized`.
