...
- [ ] Approve the TX Mini source mappings and controlled transformations.
  Blocker:
  Source-column rulings are complete, but four items still gate this approval: (1) the `tx_sow_normalized` value map for the 16 distinct raw Tx SOW values, (2) the canonical schema/validator extension decision for the newly ruled `subcontractor_tss` field, (3) implementation plus tests for the approved PR-reference-to-status transform, and (4) business confirmation that the external DU exports are approved sanitized fixtures. Writing the approved mappings into `tx_mini_pr_v1.yaml` follows the controlled profile-change process after these close.
