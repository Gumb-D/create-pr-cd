...
- [ ] Update `tx_mini_pr_v1` from `DRAFT` to a controlled validation profile only after review.
  Blocker:
  The machine-checked transition review still denies promotion with `COMPETING_SHORTLIST_CANDIDATES`: the keyword-level shortlist registry flags alternates (e.g. `Tx SOW` vs `TX SOW Details`) that JJ has already resolved in the profile, but the unresolved-review builder does not yet recognize human-approved profile mappings as overriding shortlist competition. Promotion needs either that builder enhancement or an explicit review-evidence pathway; lifecycle stays `DRAFT` and fail-closed meanwhile.
