...
- [ ] Approve the TX Mini source mappings and controlled transformations.
  Blocker:
  Source-column rulings are complete, but four items still gate this approval: (1) the `tx_mini_sow_normalized` value map — partially ruled on 2026-07-07 (round 3): the 16 distinct raw Tx SOW values were cross-checked against `Info/input/pr_model.xlsx`; `Cancel / Drop` is ruled `NO_PR_TRIGGER`, and `MW Remote Upgrade`, `New Starlink`, `Under NIC`, `Existing TX` (no PR-model SOW match) are ruled `REVIEW_REQUIRED` fail-closed quarantine, while identity normalization for the 11 exact PR-model-matching values remains `NEEDS_BUSINESS_DECISION`; (2) implemented on 2026-07-07 — the Canonical PR Site Record now carries an optional `subcontractor_tss` field (`scripts/canonical_site_validator.py` FIELD_PATHS + record shape) with adapter provenance, without changing any scope-required rule; (3) implemented on 2026-07-07 — `scripts/du_export_adapter.py` now provides the controlled `normalize_pr_reference_status` transform (non-blank reference => `PR_EXISTS`, `No PR required...` => `NO_PR_REQUIRED`, blank => `NO_PR`), covered by `tests/test_du_export_adapter.py`; full suite `Ran 191 tests` `OK` on 2026-07-07; and (4) business confirmation that the external DU exports are approved sanitized fixtures. Items (1) and (4) remain the open gates. Writing the approved mappings into `tx_mini_pr_v1.yaml` follows the controlled profile-change process after these close.
