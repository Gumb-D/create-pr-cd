# MW DU Structure Grouping Review

Discovery-only structural overlap review based on exact four-layer fingerprint sets. This is reuse guidance, not approval for shared production profiles.

## 2023 Celcomdigi BAU

- Source file: `A-P202202168750_D002-2023 Celcomdigi BAU-2023 Celcomdigi BAU_(TX_PRPO)-20260714150843.xlsx`
- Header hash: `b99438cd67273e01bba5e641a494f001295125e598abe090d3d215fedd7e2454`
- Fingerprint count: `2424`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.835` shared=`2220` union=`2659` profile=`celcomdigi_usp_pr_v1`
  - `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx` (2024 Celcomdigi BAU) similarity=`0.834` shared=`2207` union=`2645` profile=`celcomdigi_bau_2024_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.829` shared=`2183` union=`2632` profile=`tx_mini_pr_v1`

## 2023 TX Rollout

- Source file: `A-P202202168750_D002-2023 TX Rollout-TX Rollout PR_PO View-20260703160446.xlsx`
- Header hash: `8aab4c2da2dc133e0a65b9203c62e6db1ebeb30430f9f63f5c5de1673703c320`
- Fingerprint count: `2385`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.858` shared=`2205` union=`2571` profile=`tx_mini_pr_v1`
  - `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx` (2024 Celcomdigi BAU) similarity=`0.836` shared=`2192` union=`2621` profile=`celcomdigi_bau_2024_pr_v1`
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.831` shared=`2196` union=`2644` profile=`celcomdigi_usp_pr_v1`

## 2024 Celcomdigi BAU

- Source file: `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx`
- Header hash: `b3677457da49e5de484976c3fdb7ad6f5dc19055f5339ec616407f5cbde89a86`
- Fingerprint count: `2428`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.872` shared=`2275` union=`2608` profile=`celcomdigi_usp_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.862` shared=`2231` union=`2588` profile=`tx_mini_pr_v1`
  - `A-P202202168750_D002-CD consolidation 2023-CD consolidation 2023 Rollout-20260703160351.xlsx` (CD consolidation 2023) similarity=`0.842` shared=`2225` union=`2644` profile=`cd_consolidation_2023_rollout_pr_v1`

## CD consolidation 2023

- Source file: `A-P202202168750_D002-CD consolidation 2023-CD 2023 Decom Site-20260703160415.xlsx`
- Header hash: `b86cbc349db66154324092c843593137e83908c3b4b55c09305d6cf6046c7a16`
- Fingerprint count: `2431`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-CD consolidation 2023-CD consolidation 2023 Rollout-20260703160351.xlsx` (CD consolidation 2023) similarity=`0.870` shared=`2267` union=`2605` profile=`cd_consolidation_2023_rollout_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.850` shared=`2215` union=`2607` profile=`tx_mini_pr_v1`
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.842` shared=`2234` union=`2652` profile=`celcomdigi_usp_pr_v1`

## CD consolidation 2023

- Source file: `A-P202202168750_D002-CD consolidation 2023-CD consolidation 2023 Rollout-20260703160351.xlsx`
- Header hash: `d16d92debc1cc59aacd548a100d407462c7733f1894453b195abc9d3072ec9a1`
- Fingerprint count: `2441`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-CD consolidation 2023-CD 2023 Decom Site-20260703160415.xlsx` (CD consolidation 2023) similarity=`0.870` shared=`2267` union=`2605` profile=`cd_consolidation_2023_decom_pr_v1`
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.850` shared=`2249` union=`2647` profile=`celcomdigi_usp_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.848` shared=`2217` union=`2615` profile=`tx_mini_pr_v1`

## Celcomdigi USP

- Source file: `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx`
- Header hash: `79084b19ff9685eb74e3cfb4c07af8c48de871328884618e63969a623fb384cf`
- Fingerprint count: `2455`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx` (2024 Celcomdigi BAU) similarity=`0.872` shared=`2275` union=`2608` profile=`celcomdigi_bau_2024_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.852` shared=`2230` union=`2616` profile=`tx_mini_pr_v1`
  - `A-P202202168750_D002-CD consolidation 2023-CD consolidation 2023 Rollout-20260703160351.xlsx` (CD consolidation 2023) similarity=`0.850` shared=`2249` union=`2647` profile=`cd_consolidation_2023_rollout_pr_v1`

## Jendela TX Migration

- Source file: `A-P202202168750_D002-Jendela TX Migration-Migration Rollout (TX)-20260804195447.xlsx`
- Header hash: `f45c209df5ca75b333f9b590ebc01c05c097e44231d22433290f8078e57c9056`
- Fingerprint count: `2468`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-2023 Celcomdigi BAU-2023 Celcomdigi BAU_(TX_PRPO)-20260714150843.xlsx` (2023 Celcomdigi BAU) similarity=`0.657` shared=`1940` union=`2952` profile=`celcomdigi_bau_2023_pr_v1`
  - `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx` (2024 Celcomdigi BAU) similarity=`0.635` shared=`1901` union=`2995` profile=`celcomdigi_bau_2024_pr_v1`
  - `A-P202202168750_D002-2023 TX Rollout-TX Rollout PR_PO View-20260703160446.xlsx` (2023 TX Rollout) similarity=`0.633` shared=`1882` union=`2971` profile=`tx_rollout_2023_pr_v1`

## TX Mini Project

- Source file: `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx`
- Header hash: `167645031ac3ebb90da748c42fe3188ef4a67604eb0ce2c3df446df1142b5221`
- Fingerprint count: `2391`
- Grouping signal: `POSSIBLE_REUSE_REVIEW`
- Reason: Closest neighbor shares a strong majority of exact four-layer fingerprints.
- Closest neighbors:
  - `A-P202202168750_D002-2024 Celcomdigi BAU-2024 BAU Rollout (TX)-20260703160253.xlsx` (2024 Celcomdigi BAU) similarity=`0.862` shared=`2231` union=`2588` profile=`celcomdigi_bau_2024_pr_v1`
  - `A-P202202168750_D002-2023 TX Rollout-TX Rollout PR_PO View-20260703160446.xlsx` (2023 TX Rollout) similarity=`0.858` shared=`2205` union=`2571` profile=`tx_rollout_2023_pr_v1`
  - `A-P202202168750_D002-Celcomdigi USP-Celcomdigi USP (TX)-20260703160234.xlsx` (Celcomdigi USP) similarity=`0.852` shared=`2230` union=`2616` profile=`celcomdigi_usp_pr_v1`

## MW EOS Swap

- Source file: `A-P202211283695_D002-MW EOS Swap-MW EOS Swap Rollout-20260703160307.xlsx`
- Header hash: `46e50e91db7b29f9e875fabfffdd170c75739aaa39b19542a42eecf1e3d88a1a`
- Fingerprint count: `603`
- Grouping signal: `PARTIAL_REUSE_REVIEW`
- Reason: Closest neighbor shares some structure, but differences are large enough to require careful profile comparison.
- Closest neighbors:
  - `A-P202211283695_D002-ZTE TX MINI-ZTE TX MINI v1-20260703160312.xlsx` (ZTE TX MINI) similarity=`0.492` shared=`373` union=`758` profile=`zte_tx_mini_pr_v1`
  - `A-P202202168750_D002-2023 TX Rollout-TX Rollout PR_PO View-20260703160446.xlsx` (2023 TX Rollout) similarity=`0.086` shared=`236` union=`2752` profile=`tx_rollout_2023_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.086` shared=`236` union=`2758` profile=`tx_mini_pr_v1`

## ZTE TX MINI

- Source file: `A-P202211283695_D002-ZTE TX MINI-ZTE TX MINI v1-20260703160312.xlsx`
- Header hash: `a1b2f9d28ca32e38c7dbd0064602a30b9727548dfce1f1f583a961781c9be810`
- Fingerprint count: `528`
- Grouping signal: `PARTIAL_REUSE_REVIEW`
- Reason: Closest neighbor shares some structure, but differences are large enough to require careful profile comparison.
- Closest neighbors:
  - `A-P202211283695_D002-MW EOS Swap-MW EOS Swap Rollout-20260703160307.xlsx` (MW EOS Swap) similarity=`0.492` shared=`373` union=`758` profile=`mw_eos_swap_pr_v1`
  - `A-P202202168750_D002-2023 TX Rollout-TX Rollout PR_PO View-20260703160446.xlsx` (2023 TX Rollout) similarity=`0.088` shared=`236` union=`2677` profile=`tx_rollout_2023_pr_v1`
  - `A-P202202168750_D002-TX Mini Project-TX Mini PR_PO View-20260703160246.xlsx` (TX Mini Project) similarity=`0.088` shared=`236` union=`2683` profile=`tx_mini_pr_v1`
