# MW DU Profile Transition Review

Discovery-only lifecycle promotion review for the current priority DU profiles.

## tx_mini_pr_v1 (TX Mini Project)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-07-tx-mini-v1`
- Observed header hash: `167645031ac3ebb90da748c42fe3188ef4a67604eb0ce2c3df446df1142b5221`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## mw_eos_swap_pr_v1 (MW EOS Swap)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-10-mw-eos-swap-v2`
- Observed header hash: `46e50e91db7b29f9e875fabfffdd170c75739aaa39b19542a42eecf1e3d88a1a`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## tx_rollout_2023_pr_v1 (2023 TX Rollout)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-10-2023-tx-rollout-v2`
- Observed header hash: `8aab4c2da2dc133e0a65b9203c62e6db1ebeb30430f9f63f5c5de1673703c320`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## jendela_tx_migration_pr_v1 (Jendela TX Migration)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-13-jendela-tx-migration-v1`
- Observed header hash: `904f30b6c4278c0d4c20d7898f4ad3d805e9d2ca2167499ea4e9418b1a16ffe3`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## zte_tx_mini_pr_v1 (ZTE TX MINI)

- Current status: `DRAFT`
- Mapping version: `discovery-2026-07-06-zte-tx-mini-v1`
- Observed header hash: `a1b2f9d28ca32e38c7dbd0064602a30b9727548dfce1f1f583a961781c9be810`
- `PROFILED`: `DENIED` because `NO_PROFILE_SELECTION_FIELDS`
- `BUSINESS_VALIDATED`: `DENIED` because `NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, NO_PROFILE_SELECTION_FIELDS`
- `PR_INPUT_READY`: `DENIED` because `NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, NO_PROFILE_SELECTION_FIELDS`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION, NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, NO_PROFILE_SELECTION_FIELDS`

## celcomdigi_bau_2023_pr_v1 (2023 Celcomdigi BAU)

- Current status: `DRAFT`
- Mapping version: `discovery-2026-07-14-2023-celcomdigi-bau-tx-prpo-v2`
- Observed header hash: `b99438cd67273e01bba5e641a494f001295125e598abe090d3d215fedd7e2454`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `DENIED` because `NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES`
- `PR_INPUT_READY`: `DENIED` because `NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION, NO_APPROVED_HEADER_HASH, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS`

## celcomdigi_bau_2024_pr_v1 (2024 Celcomdigi BAU)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-10-2024-celcomdigi-bau-v2`
- Observed header hash: `b3677457da49e5de484976c3fdb7ad6f5dc19055f5339ec616407f5cbde89a86`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## celcomdigi_usp_pr_v1 (Celcomdigi USP)

- Current status: `PR_INPUT_READY`
- Mapping version: `approved-2026-07-10-celcomdigi-usp-v2`
- Observed header hash: `79084b19ff9685eb74e3cfb4c07af8c48de871328884618e63969a623fb384cf`
- `PROFILED`: `ELIGIBLE`
- `BUSINESS_VALIDATED`: `ELIGIBLE`
- `PR_INPUT_READY`: `ELIGIBLE`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION`

## cd_consolidation_2023_decom_pr_v1 (CD consolidation 2023)

- Current status: `DRAFT`
- Mapping version: `discovery-2026-07-07-cd-consolidation-2023-decom-v1`
- Observed header hash: `b86cbc349db66154324092c843593137e83908c3b4b55c09305d6cf6046c7a16`
- `PROFILED`: `DENIED` because `MISSING_REQUIRED_FIELDS`
- `BUSINESS_VALIDATED`: `DENIED` because `NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES`
- `PR_INPUT_READY`: `DENIED` because `NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, CROSS_MODEL_BRIDGE_ONLY_FIELDS`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION, NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, CROSS_MODEL_BRIDGE_ONLY_FIELDS`

## cd_consolidation_2023_rollout_pr_v1 (CD consolidation 2023)

- Current status: `DRAFT`
- Mapping version: `discovery-2026-07-07-cd-consolidation-2023-rollout-v1`
- Observed header hash: `d16d92debc1cc59aacd548a100d407462c7733f1894453b195abc9d3072ec9a1`
- `PROFILED`: `DENIED` because `MISSING_REQUIRED_FIELDS`
- `BUSINESS_VALIDATED`: `DENIED` because `NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES`
- `PR_INPUT_READY`: `DENIED` because `NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, CROSS_MODEL_BRIDGE_ONLY_FIELDS`
- `PRODUCTION`: `DENIED` because `PROFILE_NOT_PRODUCTION, NO_APPROVED_HEADER_HASH, MISSING_REQUIRED_FIELDS, REQUIRED_FIELDS_NOT_APPROVED, COMPETING_SHORTLIST_CANDIDATES, UNVERIFIED_SINGLE_CANDIDATE_FIELDS, CROSS_MODEL_BRIDGE_ONLY_FIELDS`
