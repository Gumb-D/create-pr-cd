...
class TestBuildTxMiniScopeUat(unittest.TestCase):
    ...
    def test_successful_run_writes_manifest_with_config_version_and_status(self):
        ...
        self.assertIn("generator_commit_sha", manifest)
        self.assertIn("source_file_name", manifest)
        self.assertIn("source_sha256", manifest)
        self.assertIn("header_hash", manifest)
        self.assertIn("profile_id", manifest)
        self.assertIn("profile_version", manifest)
        self.assertIn("mapping_version", manifest)
        self.assertIn("scope_config_path", manifest)
        self.assertIn("resolved_tss_fingerprint", manifest)
        self.assertIn("resolved_tss_column", manifest)
        self.assertIn("resolved_ti_fingerprint", manifest)
        self.assertIn("resolved_ti_column", manifest)
